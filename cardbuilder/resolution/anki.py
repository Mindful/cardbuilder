...
class AkpgResolver(Resolver):
    ...
    def _output_file(self, rows: List[CardData], name: str):
        ...
        for row in rows:
            fields = (rf.value if rf.source_name != AUDIO else '[sound:{}]'.format(rf.value) for rf in row.fields)
        ...
        if next((rf for rf in sample_row.fields if rf.source_name == AUDIO), None) is not None:
